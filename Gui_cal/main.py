...
class MainApp(App):
    def build(self):
        ...
        for row in buttons:
            ...
            for label in row: #use for arranging the buttons in screem
                button = Button(
                    text=label, font_size=30, background_color="grey", pos_hint={"center_x":0.5,"center_y":0.5}
                   
                )
                button.bind(on_press = self.on_button_press)  # help to make button working

                h_layout.add_widget(button)
            main_layout.add_widget(h_layout)

        equal_button=Button(
            text="=", font_size=30, background_color="grey", pos_hint={"center_x":0.5,"center_y":0.5}
        )
